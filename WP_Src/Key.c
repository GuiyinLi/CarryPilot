...
void Key_Init(void)
{
  ...
  // Once we have enabled (unlocked) the commit register then re-lock itdth'10\eth1\
...
bool Key_Scan(uint8_t release)
{
  ...
  if(QuadKey2==0)
  {
    ...
    if(QuadKey2==0)
    {
      ...
      Page_Number--;
			if(sins_flag==1)
				realse_lock_status=1;  //开始进行解锁 前提是高度融合完成
...
void QuadShow()
{
  ...
  if(Page_Number==0)
  {
    ...
    LCD_clear_L(0,5);  LCD_P6x8Str(0,5,"US/TF:");           if(Ground_Sensor_Now_Mode==US100) write_6_8_number(40,5,US_Distance); else write_6_8_number(40,5,tfdata.distance); write_6_8_number(80,5,NamelessQuad.Position[_YAW]);
...
//    else if(Controler_High_Mode==2&&Controler_Horizontal_Mode==2)   LCD_P6x8Str(30,7,"Auto_Ctrl_Hover");  //没有GPS
	...
	else if(Page_Number==16)
  {
      ...
      LCD_CLS();
		...
		LCD_P8x16Str(24,0,"Task Finish"); 
		LCD_P8x16Str(0,4,"Dis:");write_8_16_number(40,4,yellow_barcode_pos);
		LCD_P8x16Str(100,4,"cm");

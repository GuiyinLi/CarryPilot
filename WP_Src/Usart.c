...
#include "Usart.h"
#include "Ringbuf.h"
#define DEBUG_DATA US_TF_DATA
//主串口 靠近电源 USART0 山外地面站 看数据波形
//串口循环队列缓冲数据定义
...
{		
  ...
  UARTIntClear(UART3_BASE,flag);//清除中断标志			
	if(UARTCharGet(UART3_BASE)==0x01)
		yellow_barcode=1;
	else
		yellow_barcode=0;
...
void ConfigureUART7(void)//串口7初始化
{
  ...
  GPIOPinTypeUART(GPIO_PORTE_BASE, GPIO_PIN_0 | GPIO_PIN_1);//GPIO的UART模式配置
	...
	//ReadFlashParameterOne(GROUND_DISTANCE_DEFAULT,&ground_sensor_default);  //不从FLASH中读取测高模式
...
void ANO_DT_Data_Receive_Anl(u8 *data_buf,u8 num)//ANO数据解析
{
  ...
  if(*(data_buf+2)==0X10)                             //接收PID1   1 2 3
  {
    Total_Controller.Roll_Gyro_Control.Kp  = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );   //PID 1   ROLL角速度环
    ...
    Total_Controller.Pitch_Gyro_Control.Kp   = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) );  //PID 2 PITCH角速度环
    ...
    Total_Controller.Yaw_Gyro_Control.Kp    = 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) );   //PID 3 YAW角速度环
  ...
  if(*(data_buf+2)==0X11)                             //接收PID2   4 5 6
  {
    Total_Controller.Roll_Angle_Control.Kp  = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );    //PID 4  ROLL角度环
    ...
    Total_Controller.Pitch_Angle_Control.Kp   = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) ); //PID 5 PITCH角度环
    ...
    Total_Controller.Yaw_Angle_Control.Kp    = 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) ); //PID 6  YAW角度环
  ...
  if(*(data_buf+2)==0X12)                             //接收PID3   7 8 9
  {
    Total_Controller.High_Speed_Control.Kp    = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );   //PID 7  高度速度环
    ...
    Total_Controller.High_Position_Control.Kp = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) ); //PID 8  高度位置环
    ...
    Total_Controller.Latitude_Speed_Control.Kp= 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) );  //PID 9  经纬度速度环
  ...
  if(*(data_buf+2)==0X13)                             //接收PID4   10 11 12
  {
    Total_Controller.Latitude_Position_Control.Kp    = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );  //PID 10  经纬度位置环
    ...
    Total_Controller.High_Acce_Control.Kp            = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) );  //PID11  高度加速度环
    ...
    /***********************位置控制：位置、速度参数共用一组PID参数**********************************************************/
    Total_Controller.Longitude_Position_Control.Kp=Total_Controller.Latitude_Position_Control.Kp;        //PID 12
  ...
  if(*(data_buf+2)==0X14)                             //接收PID5  13 14 15
  {
    Total_Controller.Optical_Position_Control.Kp = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );  //PID 13 光流位置环
    ...
    Total_Controller.Optical_Speed_Control.Kp = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) );   //PID 14 光流速度环
    ...
    Total_Controller.SDK_Roll_Position_Control.Kp = 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) );  //PID 15 OPENMV 位置环
    ...
    Total_Controller.SDK_Pitch_Position_Control.Kp = 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) ); //PID 15
...
/*
  ...
  DataBuf[6]=WP_Sensor.baro_altitude_div;
	...
	#elif DEBUG_DATA==OPT_DATA
  //机头方向为Y轴 向前为正  机头左侧为X轴 向左为正   
	DataBuf[0]=opt_gyro_data.x;                  //光流低通滤波数据 之后的 原始光流角速度
  DataBuf[1]=opt_gyro_data.y;                  //光流低通滤波数据 之后的 原始光流角速度
  DataBuf[2]=opt_gyro_filter_data.x;           //原始旋转补偿滤波数据
  DataBuf[3]=opt_gyro_filter_data.y;           //原始旋转补偿滤波数据
	DataBuf[4]=OpticalFlow_SINS.Speed[_PITCH];   //惯导融合速度
  DataBuf[5]=OpticalFlow_SINS.Speed[_ROLL];    //惯导融合速度
	DataBuf[6]=OpticalFlow_SINS.Position[_PITCH];//速度计算位移
	DataBuf[7]=OpticalFlow_SINS.Position[_ROLL]; //速度计算位移
  #endif
  ...
  */
  ...
  /*
    ...
    DataBuf[2]=0;
    DataBuf[3]=OpticalFlow_SINS.Speed[_PITCH];  //惯导融合速度
    DataBuf[4]=OpticalFlow_SINS.Position[_PITCH];
    DataBuf[5]=OpticalFlow_Speed.x;             //原始速度
    DataBuf[6]=OpticalFlow_Position.x;
    DataBuf[7]=OpticalFlow_SINS.Acceleration[_PITCH];   
  
	
  DataBuf[0]=gyro_filter_data.x;              //Pitch X  陀螺仪相位同步角速度
  DataBuf[1]=gyro_filter_data.y;              //Roll R   陀螺仪相位同步角速度
  DataBuf[4]=opt_filter_data.x;           
  DataBuf[5]=opt_filter_data.y;
	DataBuf[4]=opt_filter_data.x;                //光流低通滤波数据        
  DataBuf[5]=opt_filter_data.y;                //光流低通滤波数据  

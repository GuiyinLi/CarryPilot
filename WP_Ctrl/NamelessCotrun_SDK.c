...
uint8_t move_with_speed_target(float x_target,float y_target,float delta,SDK_Status *Status,uint16_t number)
{
  static float end_time=0;
	static float yaw_current_degree=0;
  Vector2f vel_target;
  Testime dt;
  ...
  else
  {
    ...
    {
			yaw_current_degree=Yaw;    //记录执行任务前的Yaw 以便进行画弧设计
    ...
    { 
      ...
      OpticalFlow_Pos_Ctrl_Expect.x=0;if(end_time-dt.Now_Time>0)
			  Total_Controller.Yaw_Angle_Control.Expect=yaw_current_degree+45*(end_time-dt.Now_Time);  //飞机边前进边转弯用
			else
				Total_Controller.Yaw_Angle_Control.Expect=yaw_current_degree+45;
      OpticalFlow_Pos_Ctrl_Expect.y=0;
      Status->Status[number].Execute_Flag=1;
...
//更改光流PID
//行驶速度：光流位移->光流速度->姿态角度->姿态角速度  更改KP
  ...
  else
  {
		if(number==0)  //执行第一次前进任务
		{
			if(pythagorous2(OpticalFlow_Pos_Ctrl_Expect.x-OpticalFlow_SINS.Position[_PITCH],
				...
				&&pythagorous2(OpticalFlow_Pos_Ctrl_Expect.x-OpticalFlow_SINS.Position[_PITCH],
                    OpticalFlow_Pos_Ctrl_Expect.y-OpticalFlow_SINS.Position[_ROLL])>=20.0f)
			{
					SDK_DT_Send_Check(0);  //快到达塔B 提醒OPENMV识别二维码
			}
...
//航向改变控制
uint8_t rotate_with_yaw_target(float yaw_target_degree,SDK_Status *Status,uint16_t number)
{
  static uint8_t end_flag=0;
	static float yaw_current_degree=0;
  Testime dt;
  ...
  {
    if(Status->Status[number].Start_Flag==0) //此函数只执行一次
    {  
      if(yaw_target_degree==0)//不进行转向
      {
				end_flag=1;
				return 0;
      }
      ...
      {
				yaw_current_degree=Yaw;  //记录转向前的角度
        end_flag=2;
      }
      Status->Status[number].Start_Flag=1;
    ...
    if(end_flag==2)
    {
      Total_Controller.Yaw_Angle_Control.Expect=yaw_current_degree+yaw_target_degree; 
			if(ABS(Yaw-(yaw_current_degree+yaw_target_degree))<5.0f)
      {
        end_flag=0;
				yaw_current_degree=0;
        Status->Status[number].Execute_Flag=1;
        Status->Status[number].End_flag=1;
        Status->Transition_Time[number]=200;
        OpticalFlow_Pos_Ctrl_Expect.x=0;
        OpticalFlow_Pos_Ctrl_Expect.y=0;
        OpticalFlow_Control_Pure(1);//完成之后，强制悬停
        Total_Controller.High_Position_Control.Expect=0;
      }
      Status->Status[number].Execute_Flag=1;
    } 
...
//#define NCQ_SDK_DUTY1 move_with_speed_target(10,0,6000 ,&SDK_Duty_Status,1-1)//左60cm
//#define NCQ_SDK_DUTY2 move_with_speed_target(0,10,36500 ,&SDK_Duty_Status,2-1)//前365cm
//#define NCQ_SDK_DUTY3 move_with_speed_target(-10,0,36500,&SDK_Duty_Status,3-1)//右
//#define NCQ_SDK_DUTY4 move_with_speed_target(0,-10,6000,&SDK_Duty_Status,4-1)//后

#define NCQ_SDK_DUTY_MAX   6
#define NCQ_SDK_DUTY1     move_with_z_target(100,0,0,&SDK_Duty_Status,1-1)
#define NCQ_SDK_DUTY2     move_with_xy_target(0,300,&SDK_Duty_Status,2-1)      //前进length
#define NCQ_SDK_DUTY3     move_with_xy_target(60,0,&SDK_Duty_Status,3-1)       //左width
#define NCQ_SDK_DUTY4     move_with_xy_target(0,-300,&SDK_Duty_Status,4-1)      //后退length
#define NCQ_SDK_DUTY5     move_with_xy_target(-60,0,&SDK_Duty_Status,5-1)      //右width
#define NCQ_SDK_DUTY6     move_with_z_target(-100,0,0,&SDK_Duty_Status,6-1)

#define RECTANGLE_DUTY_MAX 15
SDK_Status RECTANGLE_DUTY_Duty_Status;
uint16_t RECTANGLE_Duty_Cnt=0;
uint16_t RECTANGLE_Transition_Time=0;
uint8 yellow_barcode=0;
float yellow_barcode_pos=0;
uint8_t forward_flag=0;
//飞矩形 0: 逆时针 1：顺时针
void rectangle_duty(int length,int width,uint8 mode)
{
	static int wait_time=200,hung_time=2000;
	if(RECTANGLE_DUTY_Duty_Status.Transition_Time[RECTANGLE_Duty_Cnt]>=1) 
    RECTANGLE_DUTY_Duty_Status.Transition_Time[RECTANGLE_Duty_Cnt]--;//安排过渡时间
  
  if(RECTANGLE_DUTY_Duty_Status.Status[RECTANGLE_Duty_Cnt].Start_Flag==1
     &&RECTANGLE_DUTY_Duty_Status.Status[RECTANGLE_Duty_Cnt].Execute_Flag==1
       &&RECTANGLE_DUTY_Duty_Status.Status[RECTANGLE_Duty_Cnt].End_flag==1
         &&RECTANGLE_DUTY_Duty_Status.Transition_Time[RECTANGLE_Duty_Cnt]==0)
    RECTANGLE_Duty_Cnt++;
  
  if(RECTANGLE_Duty_Cnt>=RECTANGLE_DUTY_MAX) RECTANGLE_Duty_Cnt=RECTANGLE_DUTY_MAX;
	if(Hung_Thing==1)
	{
		hung_time--;
		if(hung_time<=0)
	...
	{
		if(yellow_barcode==1&&yellow_barcode_pos==0)  //OPENMV提醒检测到黄色异物 记录此时异物据出发点的距离
		{ 
			yellow_barcode_pos=ABS(OpticalFlow_SINS.Position[_ROLL]-yellow_barcode_pos);
		}
		if(mode==0)  //逆时针
		{		
			//更改降落方式 
	    //旋转飞矩形	
			if(RECTANGLE_Duty_Cnt==0)        {move_with_xy_target(0,length,&RECTANGLE_DUTY_Duty_Status,1-1),forward_flag=1;}     //前进length
			else if(RECTANGLE_Duty_Cnt==1)   {move_with_z_target(20,0,0,&RECTANGLE_DUTY_Duty_Status,2-1),forward_flag=0;   }     //升高35cm
			else if(RECTANGLE_Duty_Cnt==2)   {rotate_with_yaw_target(90,&RECTANGLE_DUTY_Duty_Status,3-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==3)   {move_with_xy_target(0,width,&RECTANGLE_DUTY_Duty_Status,4-1),forward_flag=0; }     //前进width
			else if(RECTANGLE_Duty_Cnt==4)   {rotate_with_yaw_target(90,&RECTANGLE_DUTY_Duty_Status,5-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==5)   {move_with_xy_target(0,length,&RECTANGLE_DUTY_Duty_Status,6-1),forward_flag=1;}     //前进length
			else if(RECTANGLE_Duty_Cnt==6)   {move_with_z_target(-90,0,0,&RECTANGLE_DUTY_Duty_Status,7-1),forward_flag=0;   }     //先降落一段距离 然后着陆
			//更改此处返回降落点
		/*	else if(RECTANGLE_Duty_Cnt==6)   {rotate_with_yaw_target(90,&RECTANGLE_DUTY_Duty_Status,7-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==7   {move_with_xy_target(0,width,&RECTANGLE_DUTY_Duty_Status,8-1),forward_flag=1; }     //前进width
			else if(RECTANGLE_Duty_Cnt==8)   {rotate_with_yaw_target(90,&RECTANGLE_DUTY_Duty_Status,9-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==9)   {move_with_z_target(-90,0,0,&RECTANGLE_DUTY_Duty_Status,10-1),forward_flag=0;   }     //先降落一段距离 然后着陆*/
			else
			{
				wait_time--;
				if(wait_time<=0)
	    ...
	    //旋转飞矩形	
			if(RECTANGLE_Duty_Cnt==0)        {move_with_xy_target(0,length,&RECTANGLE_DUTY_Duty_Status,1-1),forward_flag=1;}     //前进length
			else if(RECTANGLE_Duty_Cnt==1)   {move_with_z_target(40,0,0,&RECTANGLE_DUTY_Duty_Status,2-1),forward_flag=0;   }     //升高35cm
			else if(RECTANGLE_Duty_Cnt==2)   {rotate_with_yaw_target(-90,&RECTANGLE_DUTY_Duty_Status,3-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==3)   {move_with_xy_target(0,width,&RECTANGLE_DUTY_Duty_Status,4-1),forward_flag=1; }     //前进width
			else if(RECTANGLE_Duty_Cnt==4)   {rotate_with_yaw_target(-90,&RECTANGLE_DUTY_Duty_Status,5-1),forward_flag=0;   }     //左转90
			else if(RECTANGLE_Duty_Cnt==5)   {move_with_xy_target(0,length,&RECTANGLE_DUTY_Duty_Status,6-1),forward_flag=1;}     //前进length
			else if(RECTANGLE_Duty_Cnt==6)   {move_with_z_target(-90,0,0,&RECTANGLE_DUTY_Duty_Status,7-1),forward_flag=0;   }     //先降落一段距离 然后着陆
				//更改此处返回降落点
...
{
  uint16_t i=0;
  for(i=0;i<RECTANGLE_DUTY_MAX;i++)
  {
    RECTANGLE_DUTY_Duty_Status.Status[i].Start_Flag=0;
    RECTANGLE_DUTY_Duty_Status.Status[i].Execute_Flag=0;
    RECTANGLE_DUTY_Duty_Status.Status[i].End_flag=0;
  }
  RECTANGLE_Duty_Cnt=0;
}
...
void Task_Reset(void)
{
	 //标志位原始化
		Start_Fly=0;
		Start_Task_Flag=0;           //关闭任务开始标志位
		Controler_High_Mode=1;       //关闭定高 开启自稳
		Reserve_Mode=1;              //关闭光流
		Controler_Horizontal_Mode=1; //关GPS
    SDK_Take_Over_Ctrl = 2;      //切回自定义SDK模式 
		SDK_Ctrl_Mode=0;             //关闭自定义SDK模式
		realse_lock_status=0;
	  Sun_Sky=0;
	  Hung_Thing=0;
		//清空巡线过程中的所有数据
		NCQ_SDK_Reset();
...
//    SDK_DT_Send_Data(sdk_data_to_send, 7);
	sdk_data_to_send[0]=0x01;
  SDK_DT_Send_Data(sdk_data_to_send, 1);
}
